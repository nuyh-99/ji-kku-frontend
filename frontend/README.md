...
아래는 팀 전체가 맞춰야 하는 것들이라 각자 바꾸지 마세요. 바꿔야 할 이유가 생기면 먼저 얘기해주세요.

- **스타일링은 Tailwind 로 통일합니다.** styled-components 같은 CSS-in-JS 를 섞지 않습니다.
  두 체계가 공존하면 같은 걸 두 가지 방법으로 하게 되고, 번들도 커지고, App Router 에서는
  CSS-in-JS 가 SSR 레지스트리 설정을 따로 요구합니다.
- **서버 상태는 TanStack Query, 클라이언트 UI 상태는 Zustand.** (4번 상태 경계 참고)
- **HTTP 호출은 `lib/api/client.ts` 의 `apiFetch`.** fetch/axios 를 각자 쓰지 않습니다.

> 그 밖의 라이브러리(예: 지도 SDK, 날짜 유틸)는 각 담당이 필요에 따라 도입해도 됩니다.
> 다만 의존성을 추가하면 `package.json` 이 충돌하기 쉬우니, 추가한 날 바로 작은 PR 로 올려주세요.
